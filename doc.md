...
The `pushd` builtin adds directories to the stack as it changes the current directory, and the `popd` builtin removes specified directories from the stack and changes the current directory to the directory removed.
...
### 1. Go to the instructed directory
```bash
	log "Begin ${SUB_STAGE_DIR}"
	pushd "${SUB_STAGE_DIR}" > /dev/null
```

### 2. For each directoy look for the files of form `\d\d-debconf`
### 2.1 For each directoy look for the files of form `\d\d-debconf`
```bash
on_chroot << EOF <br/>
...
EOF
```
### 2.1 For each directoy look for the files of form `${i}-packages-nr`
...
```
### 2.3 For each directoy look for the files of form `${i}-packages-nr`
...
```
### 2.3 For each directoy look for the files of form `${i}-packages`
```Bash
...
```
### 2.3 For each directoy look for the files of form `${i}-run.sh`
```bash
